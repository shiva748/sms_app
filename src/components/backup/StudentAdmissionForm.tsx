...
export const StudentAdmissionForm: React.FC<StudentAdmissionFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      submitData.append("data", JSON.stringify({ ...formData, id: selectedStudent.student ? selectedStudent.student.id : null, photo: null }));
      try {
        const req = await fetch(`${API}/schools/${school.id}/students`, {
          method: selectedStudent.student ? "PUT" : "POST",
          credentials: "include",
          headers: {
